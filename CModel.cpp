...
void CModel::createModel(tntdb::Connection conn, string table) {
    ...
    if (gen_f_model_h.is_open()) {
        ...
        gen_f_model_h << "#ifndef _GEN_MODEL_" << boost::to_upper_copy(table) << "_H\n"
                ...
                "\tbool _create(tntdb::Connection conn);\n"
                "\tbool _update(tntdb::Connection conn);\n"
                "\tbool _delete(tntdb::Connection conn);\n"
                "\tbool _read(tntdb::Connection conn);\n\n"
                << setFunction <<
                "\n"
                << getFunction <<
                "private:\n"
    ...
    if (gen_f_model_cpp.is_open()) {
        ...
        gen_f_model_cpp << "bool gen_model_"
                ...
                "\t\t}\n"
                "\t}\n}\n\n"
                "bool gen_model_" << table << "::_update(tntdb::Connection conn){\n"
                "\tif(this->isDirty){\n\t\ttry{\n"
                ...
                "\t\t}\n"
                "\t}\n}\n\n"
                "bool gen_model_" << table << "::_delete(tntdb::Connection conn){\n"
                "\ttry{\n"
                ...
                "\t}\n"
                "}\n\n"
                "bool gen_model_" << table << "::_read(tntdb::Connection conn){\n"
                "\ttry{\n"
